...
5. Structure your answer: brief direct answer → supporting cases → any caveats.
  ...
  try {
    ...
    if (!chunks || chunks.length === 0) {
      return NextResponse.json({
        answer: "No relevant cases found for your query. Try different search terms.",
        sources: [],
      });
    }
    ...
    // 5. Fetch case metadata for sources
    const uniqueCitations = [...new Set(chunks.map((c: any) => c.citation))];
    const { data: caseData } = await sb
      .from("cases")
      .select(
        "citation, court, date_decision, headnote, subject_matters, elitigation_url, url, outcome_winner, is_appellate"
      )
      .in("citation", uniqueCitations);
    ...
    const sources = chunks.map((c: any) => {
      ...
      return {
        ...
        court: c.court,
        year: c.year,
        is_appellate: meta.is_appellate,
